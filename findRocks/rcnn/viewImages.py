...
def applyToAllRows(pathToRead):
    ...
    formatBetter = formatBetter.assign(Class = 'Boulder')
...
def readInAllExportToOneCSV(oldPath = "C:/Users/jlombardi/Documents/GitLabCode/nnplay/findRocks/data/allData/labels_wrongFormat/",
                 newPath = "C:/Users/jlombardi/Documents/GitLabCode/nnplay/findRocks/data/allData/labels/"):
    ...
    for i in range(len(fileCompleteList)):
        bboxData = applyToAllRows(fileCompleteList[i])
        bboxData['fileName'] = fileCompleteList[i]
        subString = fileCompleteList[i]
        subString = subString[subString.rindex('/')+1:]
        bboxData['UniqueImageID'] = subString.split('.')[0]
        newData = newData.append(bboxData)

    newData.to_csv(newPath + 'allData.csv',
                   header=list(newData.columns),
                   index=None, sep=',', mode='a')

    return None

readInAllExportToOneCSV()
#----------------
