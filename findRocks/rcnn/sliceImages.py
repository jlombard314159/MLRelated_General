...
def get_tiles(img_size, tile_size, tile_overlap):
...
emptyIMG_DST = data_dir + 'sliced/emptyImages'
slice_images(tile_size=(300,300), dataPath=dataPath,
             IMG_DST=IMG_DST, emptyIMG_DST=emptyIMG_DST,
             imagesToNotMake=emptyImageList)
#--------------------------------------------------------------

data_dir = "C:/Users/jlombardi/Documents/GitLabCode/nnplay/findRocks/data/voc/sliced/labels/"

#Now convert XML to something useful

xml_file = sorted(glob.glob(data_dir + os.sep + '*.xml'))[0]
...
def extractXMLToTxt(xmlPath,newPath):
    ...
    outputDF.to_csv(newPath + 'allData.csv',
                   header=list(outputDF.columns),
                   index=None, sep=',', mode='a')

    return None

extractXMLToTxt(xmlPath = data_dir,
                newPath = data_dir)